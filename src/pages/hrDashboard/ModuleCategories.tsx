    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-100 py-12 px-4 animate-fadeIn">
      <div className="max-w-4xl w-full bg-white rounded-2xl shadow-2xl p-8">
        <div className="text-center mb-8">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-blue-900 tracking-tight drop-shadow mb-2">
            {getCourseDisplayName(courseName || '')} - {getModuleDisplayName(moduleName || '')}
          </h2>
          <p className="text-lg text-blue-700">Select a category to view applications</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {categories.map((category) => (
            <button
              ...
              className={`w-full py-12 px-6 bg-gradient-to-br ${category.color} rounded-xl shadow-md border ${category.borderColor} hover:shadow-xl hover:scale-105 focus:scale-105 focus:ring-2 focus:ring-blue-400 transition-all duration-200 flex flex-col items-center group relative cursor-pointer animate-slideUp`}
              tabIndex={0}
              title={category.desc}
            >
              <span className="mb-4">{category.icon}</span>
              <span className={`font-bold text-xl ${category.textColor} group-hover:scale-110 transition-transform`}>
                {category.name}
              </span>
              <span className="absolute left-1/2 -bottom-2 -translate-x-1/2 opacity-0 group-hover:opacity-100 bg-blue-900 text-white text-xs rounded px-3 py-2 pointer-events-none transition-all duration-200 z-10 shadow-lg whitespace-nowrap max-w-xs">
                {category.desc}
              </span>
            </button>
          ))}
