    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-100 py-12 px-4 animate-fadeIn">
      <div className="max-w-4xl w-full bg-white rounded-2xl shadow-2xl p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-blue-700">{subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {items.map((item) => (
            <button
              ...
              className={`w-full py-12 px-6 bg-gradient-to-br ${item.color} rounded-xl shadow-md border ${item.borderColor} hover:shadow-xl hover:scale-105 focus:scale-105 focus:ring-2 focus:ring-blue-400 transition-all duration-200 flex flex-col items-center group relative cursor-pointer animate-slideUp`}
              tabIndex={0}
              title={item.desc}
            >
              <span className="mb-4">{item.icon}</span>
              <span className={`font-bold text-xl ${item.textColor} group-hover:scale-110 transition-transform`}>
                {item.name}
              </span>
